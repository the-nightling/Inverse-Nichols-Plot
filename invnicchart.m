function [varargout] = invnicchart(varargin)
%INVNICCHART  Generates the Inverse Nichols grid.
%
%    [PHASE,GAIN,LABELS] = INVNICCHART(PMIN,PMAX,GMIN) generates the
%    data for plotting the Inverse Nichols chart. PMIN and PMAX specify the
...
%
%    [GRIDHANDLES,TEXTHANDLES] = INVNICCHART(AX)  plots the Nichols chart 
%    in the axis AX.  NICCHART uses the current axis limits.
%
%    [GRIDHANDLES,TEXTHANDLES] = INVNICCHART(AX,OPTIONS) specifies additional
%    grid options.  OPTION is a structure with fields:
%      * PhaseUnits: 'deg' or 'rad' (default = deg)
%      * Zlevel    : real scalar (default = 0)
%
%    See also NICCHART, NICHOLS, NGRID.

%   Adapted from
%   Authors: K. Gondoly and P. Gahinet
%   Copyright 1986-2003 The MathWorks, Inc.
%   $Revision: 1.1.8.1 $  $Date: 2011/11/17 21:17:40 $
...
if ni>=3
   ...
   GridHandles(1,1) = line(phase, -1*gain, Zlevel(ones(length(gain),1),:), ...
